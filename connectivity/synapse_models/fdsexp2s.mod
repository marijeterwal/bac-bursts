...
NET_RECEIVE(weight (umho), F, D1, tsyn (ms)) {
INITIAL {
...
: printf("t\t t-tsyn\t F\t D1\t amp\t newF\t newD1\n")
}

        F = 1 + (F-1)*exp(-(t - tsyn)/tau_F)
        D1 = 1 - (1-D1)*exp(-(t - tsyn)/tau_D1)
: printf("%g\t%g\t%g\t%g\t%g\t%g", t, t-tsyn, F, D1, weight*F*D1)
        tsyn = t

	state_discontinuity(A, A + weight*factor*F*D1)
	state_discontinuity(B, B + weight*factor*F*D1)
	total = total+weight*F*D1
